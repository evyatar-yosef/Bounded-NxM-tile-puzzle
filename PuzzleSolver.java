    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            if (whiteBlocksLine.equals("White")) {
                ...
                System.out.println("White blocks: " + whiteBlocks);
            }
...
//////////////////////////////////////////////////////////////////////////////////////////////////////
            ...
            if (algorithmType.equals("DFID")) {
                ...
                solverResult = AStarSolver.aStarSolve(initialPuzzleState);
            } else if (algorithmType.equals("IDA*")) {
                solverResult = IDASolver.idaStarSolve(initialPuzzleState);
            } else if (algorithmType.equals("DFBnB")) {
                solverResult = DFBnBSolver.dfbnbSolve(initialPuzzleState);
            } else {
            ...
            // Print the solution or handle it as needed
            if (solution != null) {
                System.out.println("Solution found:");
                // Print the solution
                for (PuzzleState state : solution) {
                    // Print each tile of the solution path
                    System.out.println(state);
                }
                // Print number of steps and cost
                int cost = solution.get(solution.size() - 1).getCost(); // Cost is the cost of the last state in the solution
                System.out.println("Cost: " + cost);
                // Print execution time if specified
    ...
    private static void writeSolutionToFile(List<PuzzleState> solution, double executionTime,String printTime) {
        try {
            ...
            if (printTime.equals("with time"))  {
                writer.write(executionTime + " seconds\n");
            }        
            writer.close();
        } catch (IOException e) {
            e.printStackTrace();
    ...
    private static String getAction(PuzzleState from, PuzzleState to) {
        ...
        // Calculate the action based on the movement of the empty cell
        if (fromEmptyRow < toEmptyRow) {
            return tileNumber + Math.abs(toEmptyRow - fromEmptyRow) + "D";
        } else if (fromEmptyRow > toEmptyRow) {
            return tileNumber + Math.abs(toEmptyRow - fromEmptyRow) + "U";
        } else if (fromEmptyCol < toEmptyCol) {
            return tileNumber + Math.abs(toEmptyCol - fromEmptyCol) + "R";
        } else {
            return tileNumber + Math.abs(toEmptyCol - fromEmptyCol) + "L";
        }
    }
